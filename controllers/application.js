...
exports.createApp = async (req, res) => {
  try {
    ...
    const app = await db.Application.create({...req.body, appId:req.params.id, userId: req.user.userId});
    res.status(201).json({
...
exports.addPersonalInfo = async (req, res) => {
  ...
  try {
    ...
    console.log("Data: 1 ");
    const existsCheck = await db.PersonalDetail.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };

    const data = await db.PersonalDetail.create({...req.body, appId: parseInt(req.params.id, 10), userId: req.user.userId });
    res.status(201).json({
...
exports.addEducation = async (req, res) => {
  ...
  try {
    ...
    if(!req.body.education) {
      return res.status(400).json({ msg: "Please select an School/College.", errors: [] });
    }

    const existsCheck = await db.AcadQualification.findOne({where: { appId: req.params.id, education: req.body.education }});
    if (existsCheck) {
      if(req.body.education !== "OTHER"){
        return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this education."}] });
      }
    };

    const data = await db.AcadQualification.create({...req.body, userId: req.user.userId });
    res.status(201).json({
      msg: "Data added successfully.",
      data: data,
    });
...
exports.addAcadExp = async (req, res) => {
  ...
  try {
    if(!req.body.org) {
      return res.status(400).json({ msg: "Please provide an Universiry/Org.", errors: [] });
    }

    const existsCheck = await db.AcadExperience.findOne({where: { appId: req.body.appId, org: req.body.org, designation: req.body.designation }});
    if (existsCheck) {
      if(req.body.education !== "OTHER"){
        return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this education."}] });
      }
    ...
    const data = await db.AcadExperience.create({...req.body, userId: req.user.userId });
    res.status(201).json({
      msg: "Data added successfully.",
      data: data,
    });
...
exports.addSponseredProject = async (req, res) => {
  ...
  try {
    const existsCheck = await db.Research.findOne({where: { appId: req.params.id}});
    if (existsCheck) {
        return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addThesis = async (req, res) => {
  ...
  try {
    const existsCheck = await db.Thesis.findOne({where: { appId: req.params.id}});
    if (existsCheck) {
        return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addPublications = async (req, res) => {
  ...
  try {
    const existsCheck = await db.Publications.findOne({where: { appId: req.params.id}});
    if (existsCheck) {
        return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addBestPapers = async (req, res) => {
  ...
  req.body.appId = req.params.id
    ...
    const existsCheck = await db.BestPapers.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addSOP = async (req, res) => {
  ...
  try {
    req.body.doc = req.files?.doc[0]?.path
    const existsCheck = await db.SOP.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addPatents = async (req, res) => {
  ...
  try {
    const existsCheck = await db.Patents.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };
...
exports.addOtherInfo = async (req, res) => {
  ...
  try {
    if(req.body.awards !== "")
      req.body.awards = req.files?.awards[0]?.path
    if(req.body.extraCirricular !== "")
      req.body.extraCirricular = req.files?.extraCirricular[0]?.path


    const existsCheck = await db.OtherInfo.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };

    const data = await db.OtherInfo.create({...req.body, appId: req.params.id });
    res.status(201).json({
      msg: "Data added successfully.",
      data: data,
    });
...
exports.addFuturePlans = async (req, res) => {
  ...
  try {
    req.body.doc = req.files?.doc[0]?.path
    const existsCheck = await db.FuturePlans.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };

    const data = await db.FuturePlans.create({...req.body, appId: req.params.id });
    res.status(201).json({
      msg: "Data added successfully.",
      data: data,
    });
...
exports.addGeneral = async (req, res) => {
  ...
  try {
    const existsCheck = await db.GeneralQues.findOne({where: { appId: req.params.id }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this section."}] });
    };

    const data = await db.GeneralQues.create({...req.body, appId: req.params.id });
    res.status(201).json({
      msg: "Data added successfully.",
      data: data,
    });
...
exports.addReferees = async (req, res) => {
  ...
  try {
    const existsCheck = await db.Referee.findOne({where: { email: req.body.email }});
    if (existsCheck) {
      return res.status(400).json({ msg: "Data already exists.", errors: [ {message: "You have already filled the details for this Referee."}] });
    };
